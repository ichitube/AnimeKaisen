...
async def on_bot_join(event: ChatMemberUpdated, bot: Bot):
...
async def duel_timeout(chat_id, user_id, r, mes, bot: Bot):
...
async def request_timeout(chat_id, user_id, mes, bot: Bot):
...
async def duel(message: Message, bot: Bot):
...
async def start_duel(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    chat_id = callback.message.chat.id
    rival_id = False
    if user_id in request_data[chat_id]:
...
async def duel_battle(callback: CallbackQuery, bot: Bot):
