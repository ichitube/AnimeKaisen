...
def abilities_kb(
    ...
    *,
    ...
    cancel_cb: str = "pvp_surrender",
) -> InlineKeyboardMarkup:
    """
    Клавиатура навыков:
    - abilities: список названий навыков (текст на кнопке = название)
    - callback_data = prefix + название (например: '˹Огненный шар')
    - две колонки; если нечётно — последняя в одиночку
    - в самом конце отдельная строка с кнопкой 'Сдаться'
    """
    ...
    # Две колонки, «хвост» сам уйдёт на единственную кнопку в последней строке
    builder.adjust(2)

    # Отдельной строкой — «Сдаться»
    builder.row(InlineKeyboardButton(text=cancel_text, callback_data=cancel_cb))
